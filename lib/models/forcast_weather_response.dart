class ForcastWeatherResponse {
  ...
  ForcastWeatherResponse.fromJson(dynamic json) {
    ...
    if (json['list'] != null) {
      list = [];
      json['list'].forEach((v) {
        list?.add(ForcastItem.fromJson(v));
      });
    }
  ...
  num? message;
  num? cnt;
  List<ForcastItem>? list;
  City? city;
ForcastWeatherResponse copyWith({  String? cod,
  num? message,
  num? cnt,
  List<ForcastItem>? list,
  City? city,
}) => ForcastWeatherResponse(  cod: cod ?? this.cod,
...
class ForcastItem {
  ...
  ForcastItem.fromJson(dynamic json) {
    dt = json['dt'];
